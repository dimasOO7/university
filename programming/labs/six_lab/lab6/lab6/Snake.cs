...
namespace lab
{
    ...
    class Snake
    {
        ...
        public Snake()
        {
            int menuResult = Menu_old.MenuSelection(["лёгкая", "средняя", "высокая", "собственная"], "========= Выбор Сложности =========");
            int walls;
        ...
        public void RunGame()
        {
            ...
            while (isRunning)
            {
                ...
                {
                    isRunning = Menu_old.MenuSelection(["нет", "да"], $"гг, результат: {result} \nсид был: {seed}\nПерезапустить игру на той же карте?") == 1;
                    if (isRunning)
                    {
                        RestartGame();
                    }
                ...
                else
                {
                    if (snakeSize + result == freeCells)
                    {
                        isRunning = Menu_old.MenuSelection(["нет", "да"], $"ГОООООООЛ, результат: {result} \nсид был: {seed}\nПерезапустить игру на той же карте?") == 1;
                        if (isRunning)
                        {
                            RestartGame();
                        }
